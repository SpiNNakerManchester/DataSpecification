...
class Commands(Enum):
    ...
    READ                    = (0x41, dsef.execute_read, "Performs a simple read  operation")
    WRITE                   = (0x42, dsef.execute_write, "Performs a simple write or block write operation")
    WRITE_ARRAY             = (0x43, dsef.execute_write_array, "Performs a write from an array")
    WRITE_STRUCT            = (0x44, dsef.execute_write_struct, "Performs a write from a predefined structure")
    BLOCK_COPY              = (0x45, dsef.execute_block_copy, "Copies a block of data from one area to another")
    ...
    RESET_WR_PTR            = (0x65, dsef.execute_reset_wr_ptr, "Move the write pointer at the beginning of the memory region")
    ALIGN_WR_PTR            = (0x66, dsef.execute_align_wr_ptr, "Moves the write pointer so that it points to the next block with a given address granularity")
    ...
    READ_PARAM              = (0x73, dsef.execute_read_param, "Load the value of a structure parameter in a register")
    WRITE_PARAM_COMPONENT   = (0x74, dsef.execute_write_param_component, "Modify a single parameter in a structure")
