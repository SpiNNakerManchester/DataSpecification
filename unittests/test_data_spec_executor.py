...
class TestDataSpecExecutor(unittest.TestCase):
    ...
    def test_trivial_spec(self):
        temp_spec = mktemp()
        spec = DataSpecificationGenerator(open(temp_spec, "wb"))
        spec.end_specification()

        executor = DataSpecificationExecutor(open(temp_spec, "rb"), 400)
        executor.execute()
